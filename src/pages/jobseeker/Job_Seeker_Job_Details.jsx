...
const ApplicationModal = ({ show, onClose, jobId, jobTitle }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://joblink-server-app.vercel.app/api/apps", {
        method: "POST",
        headers: {
...
function Job_Seeker_Job_Details() {
  ...
  const checkApplicationStatus = async () => {
    try {
      ...
      const response = await fetch(
        `https://joblink-server-app.vercel.app/api/apps/check?jobId=${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        setLoading(true);
        const response = await fetch(`https://joblink-server-app.vercel.app/api/jobs/${id}`);
